...
fn test_gc_trashes_expired() {
    ...
    let config = Config {
        base_path: temp_dir.path().to_path_buf(),
        saved: SavedConfig {
            trash_ttl: Duration::from_secs(100), // Short TTL for testing
            purge_ttl: Duration::from_secs(50),
...
fn test_gc_purges_expired() {
    ...
    let config = Config {
        base_path: temp_dir.path().to_path_buf(),
        saved: SavedConfig {
            trash_ttl: Duration::from_secs(100),
            purge_ttl: Duration::from_secs(50), // Short purge TTL
            large_file_threshold_bytes: 1024 * 1024,
        },
    };
    let mut db = Database::new(config).unwrap();
...
fn test_insert_blob_stored_text() {
    ...
    let value = db.get(&key).unwrap().unwrap();
    assert_eq!(value.clip_data, ClipData::Text(latest_value::TextData::BlobStored));
}
